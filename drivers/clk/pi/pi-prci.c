...
#include <dt-bindings/clock/pi-a88-prci.h>
...
{
D	return readl(pd->va + offs);
}

static void __prci_writel(u32 v, u32 offs, struct __prci_data *pd)
{
D	writel(v, pd->va + offs);
}
...
{
D	__prci_wrpll_unpack(&pwd->c, __prci_readl(pd, pwd->cfg0_offs));
}
...
static void __prci_wrpll_write_cfg0(struct __prci_data *pd,
				    ...
				    struct wrpll_cfg *c)
{
D	__prci_writel(__prci_wrpll_pack(c), pwd->cfg0_offs, pd);
...
static void __prci_wrpll_write_cfg1(struct __prci_data *pd,
				    ...
				    u32 enable)
{
D	__prci_writel(enable, pwd->cfg1_offs, pd);
}

unsigned long pi_prci_wrpll_recalc_rate(struct __prci_clock *pc,
					    unsigned long parent_rate)
{
D	struct __prci_wrpll_data *pwd = pc->pwd;
...
{
	/* Release CLTX reset */
D	__prci_consumer_reset("cltx_reset", true);
}
...
static int pi_prci_probe(struct udevice *dev)
{
D	int i, err;
	struct __prci_clock *pc;
...
{
D	return pi_reset_bind(dev, PRCI_DEVICERESETCNT);
}
...
U_BOOT_DRIVER(pi_prci) = {
	.name = "pi-prci",
	.id = UCLASS_CLK,
	.of_match = pi_prci_ids,
	.probe = pi_prci_probe,
	.ops = &pi_prci_ops,
